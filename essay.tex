...
In this essay, we present a new mechanisation of category theory\footnote{Our proof scripts are available online at \url{https://github.com/xrchz/HOL/tree/category/examples/category}, as well as in the HOL4 repository (\url{http://sf.net/projects/hol/develop}) from revision <...>}, refining two previous developments in higher-order logic, and investigate the foundational issues.
...
%}
\section{Composable Morphisms and Categories}%1563/1400%{
...
We define what it means for pair of morphisms to be inverses:
\begin{align*}
...
%}
\section{A Category of Classes and of Sets}%1469/1400%{
\label{Set}
...
If $c.\mathtt{mor}$ is itself a set, then there is no category whose morphisms are all the functions between sets because no set can contain all functions, so $\Set$ cannot be formalised.
...
We have proved that embeddings are essentially injective, and that $\mathtt{rep\_functor}\;u$ is an embedding in HOL.
...
%}
\section{The Yoneda Embedding}%790/500%{
\label{Yoneda}
...
To get a version of $\mathtt{Yfunctor}\;c$ whose codomain is $[c^\mathrm{op}\to\mathtt{set\_cat}]$, we make use $\mathtt{rep\_functor}$ as well as the isomorphism between $\mathtt{set\_cat}$ and the $\mathtt{ens\_cat}$ of small ZFC sets.
...
Using our existing theorems about $\mathtt{Yfunctor}\;c$, along with general theorems about embeddings being preserved by functor composition and by lifting to a functor category, we can quickly prove that $\mathtt{zYfunctor}\;c$ is an embedding whenever $c$ is locally small.
...
Mathematically, the Yoneda embedding ought to be injective on objects.
...
where the action of $\mathtt{tag\_functor}\;c$ on a hom $h$ is the ZFC ordered pair whose first component is the empty ZFC set if $h$ is of the form $c(x,x)$ (otherwise a singleton ZFC set), and whose second component is just the action of $\mathtt{ens\_to\_set}\circ\mathtt{rep\_functor}\;(\mathtt{homs}\;c)$ on $h$.
The ZFC sets now encode whether they are representing a hom of the form $c(x,x)$ or $c(x,y)$, therefore we can always tell functors $c(\blank,x)$ and $c(\blank,y)$ apart by their action on $x$, even if they are isomorphic functors.
...
%}
\section{Binary Products and Other Limits}%356/250%{
\label{limits}
...
<foundations an old problem, and still being worked out mathematically? \cite{Blass,springerlink:10.1007/BFb0059147,Easwaran}
%}
