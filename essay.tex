...
%}
\section{Higher-Order Logic}%500/500%{
...
The type $\alpha\to\bool$ can be abbreviated by $\alpha\;\mathsf{set}$: a predicate on $\alpha$ is the characteristic function of a set of elements of type $\alpha$.
...
%}
\section{Composable Morphisms and Categories}%1359/1400%{
...
%}
\section{A Category of Classes and of Sets}%1527/1400%{
\label{Set}
...
%}
\section{The Yoneda Embedding}%598/500%{
\label{Yoneda}
...
To get a version of $\mathtt{Yfunctor}\;c$ whose codomain is $[c^\mathrm{op}\to\mathtt{set\_cat}]$
<basically reuse the proof for set cat, to get zYfunctor, along with some theorems about embeddings being preserved etc.>

In Katovksy's development, the codomain of $\mathtt{Yfunctor\;c}$ is $\mathtt{set\_cat}$. 
Therefore, he requires $c$ to be locally small, and specifies that a category is locally small 
...
%}
\section{Binary Products and Other Limits}%365/250%{
\label{limits}
