...
%}
\section{Composable Morphisms and Categories}%687/1400%{
...
However, they use additional fields $\mathtt{dom}$ and $\mathtt{cod}$, both of type $\beta\to\alpha$, intended to assign a domain and codomain to each morphism: there is no separate type of morphisms, and maps do not have (categorical) types except in the context of a category.
...
Nevertheless, we can simply define homs after categories: $\mathtt{hom}\;c\;x\;y=\{f\mid f:x\to y\land f\in c.\mathtt{mor}\}$.
...
%}
\section{A Category of Classes and of Sets}%348/1400%{
...
This method of accessing set theory directly within the HOL logic was introduced by Mike Gordon \cite{DBLP:conf/tphol/Gordon96} and is implemented in both HOL4 and Isabelle \cite{DBLP:conf/ictac/Obua06}.
%}
