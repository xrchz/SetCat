...
\vspace{2em}
\noindent Words: 5656 (Maximum: 5000)
\vspace{5em}
%}
...
Special thanks to Roger Jones for his detailed reading and comments.
%}
\begin{abstract}%{
...
%}
\section{Higher-Order Logic}%420/400%{
...
%}
\section{Composable Morphisms and Categories}%1527/1500%{
...
%}
\section{A Category of Classes and of Sets}%1423/1400%{
\label{Set}
...
%}
\section{The Yoneda Embedding}%816/600%{
\label{Yoneda}
...
The importance of the Yoneda embedding comes from the fact that $\widehat{c}$ inherits much structure from the category of sets (for example, having small limits and colimits) so can be used when $c$ itself lacks this structure.
...
O'Keefe, who also proves the Yoneda lemma for ``($\mathtt{ens\_cat}\;u$)-valued'' presheaves, takes $u=\mathtt{UNIV}$; we instead take a universe containing only the homs of $c$ (but we define an inclusion functor that embeds $\mathtt{ens\_cat}\;u_1$ into $\mathtt{ens\_cat}\;u_2$ whenever $u_1\subseteq u_2$).
...
This category is locally small because each hom contains at most one morphism, but there is no injective representation of all functions as ZFC sets to attach to Katovsky's record.
%}
\section{Binary Products and Other Limits}%408/250%{
\label{limits}
