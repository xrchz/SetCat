...
\usepackage[a4paper,inner=1.5in,outer=1.2in]{geometry}
...
\vspace{3em}
\noindent Words: 4892%\footnote{\texttt{detex essay.tex | wc -w} after removing Declaration, Acknowledgments, Abstract, and References.}
%}
\vfill
...
In this essay we present a new mechanisation of category theory\footnote{Our proof scripts are available online at \url{https://github.com/xrchz/HOL/tree/category/examples/category}, as well as in the HOL4 repository (\url{http://sf.net/projects/hol/develop}) from revision 9128.}, refining two previous developments in higher-order logic, and investigate the foundational issues.
...
An isomorphism is a morphism with an inverse, and two objects are isomorphic if there is an isomorphism between them.
...
%}
\section{Two Categories of Sets}%1364/1400%{
\label{Set}
...
This method of accessing set theory within HOL was introduced by Mike Gordon \cite{DBLP:conf/tphol/Gordon96} (and independently by Bob Solovay, see~\cite{DBLP:conf/ictac/Obua06}) and is implemented in both HOL4 and Isabelle~\cite{DBLP:conf/ictac/Obua06}.
We use it, following Katovksy\footnote{Agerholm~\cite{Agerholm95} does a similar thing in HOL4.}, to define a non-polymorphic category of sets with $\mathtt{set\_cat}.\mathtt{obj}:\mathsf{zfset}\;\mathsf{set}$ and $\mathtt{set\_cat}.\mathtt{mor}:\mathsf{zfset}\;\mathsf{set}$ also (since functions between ZFC sets can be encoded as ZFC sets).
...
%(although it might be better to use $\mathtt{obj}:\alpha$ and a convention to only instantiate $\alpha$ by $\mathsf{zfset}\;\mathsf{set}$ or $\mathsf{zfset}\;\mathsf{set}\;\mathsf{set}$ and so on, to allow arbitrarily large categories).
This approach matches a mathematical view of categories based on set theory (e.g., \cite{MacLaneCFTWM}) if we take $\mathtt{zfset}$ seriously as our representation of sets.
...
Mathematics is traditionally encoded in set theory because it supports the encoding of almost all mathematical objects.
We can tap into this idea by representing each HOL set (of type $\alpha\;\mathsf{set}$ for some type $\alpha$) as a ZFC set.
...
Every embedding is \emph{essentially injective on objects}: if it sends two objects in the source category to the same object in the target category, then those two objects are isomorphic in the source category.
...
Indeed there are more small sets of type $\mathtt{zfrep}\;\mathsf{set}$ than ZFC sets, so $\mathtt{zfrep}$ could not be injective.
...
%}
\section{The Yoneda Embedding}%776/700%{
\label{Yoneda}
...
The Yoneda embedding sends an object $x$ in $c$ to the \emph{contravariant hom functor} $c(\blank,x)$, which in turn sends an object $y$ in $c$ to $c(y,x)$ (i.e., $\mathtt{hom}\;c\;y\;x$).
In order for $c(\blank,x)$ to be a set-valued presheaf, however, every hom $c(y,x)$ must be a set.
A category where every hom is a set is called \emph{locally small}; the Yoneda embedding is only defined for locally small categories.
However, the proof of the Yoneda lemma never mentions smallness explicitly, and the argument still goes through when we generalise from $\mathbf{Set}$ to an arbitrary category whose objects are appropriately set-like and include the homs of $c$.
...
O'Keefe only proves the Yoneda lemma for $\mathtt{ens\_cat}\;\mathtt{UNIV}$ so never needs to distinguish locally small categories.
...
Indeed, there is a locally small category (albeit using our definition of ``small'') that cannot be represented with Katovsky's approach, namely, $\mathtt{discrete\_cat}\;(\mathtt{UNIV}:\mathsf{zfset}\;\mathsf{set}\;\mathsf{set})$.
This category is locally small because each hom contains at most one morphism, but there is no injective representation of collections of ZFC sets as ZFC sets.
%}
\section{Binary Products}%313/250%{
\label{limits}
We define limits in HOL via \emph{cones}, defined in turn via \emph{comma categories}.
We define binary products as limits of a functor, called $\mathtt{product\_diagram}$, whose domain is $\mathtt{discrete\_cat}\;\{1,2\}$.
...
The theorem above says for every pair of objects $a$ and $b$ in a category $c$ there exists a canonical product object $ab$ with projections $p_1$ and $p_2$.
We use Hilbert choice to define constants $a\times b\operatorname{\mathtt{-:}}c$ , $\pi_1^{a,b,c}$, and $\pi_2^{a,b,c}$ to denote these terms whenever they exist.
...
Ours is the first mechanisation of limits in HOL (although limits are covered by mechanisations in other systems, e.g., \cite{DBLP:conf/birthday/HuetS00,Simpson04}).
The fact that $\mathtt{set\_cat}$ has binary products is reassuring for the $\mathsf{zfset}$ approach to sets.
We would not expect to be able to prove that $\mathtt{ens\_cat}\;\mathtt{UNIV}$ has products.

Using products as above, we can define a binary product functor.
One definition of \emph{exponential objects}, an important categorical construction, gives them via a functor that is \emph{right adjoint} to the binary product functor.
The best way to treat adjunctions in HOL is not obvious.
We could define exponential objects directly, ignoring the adjunction, but to do so is to miss out on opportunities for abstraction.
%}
\section{Conclusions}%354/300%{
...
\item
Since HOL functions are total, we need to restrict partial functions so that they are extensional, that is, comparable by their action on their domains only.
The need for extensionality is pervasive.
\item
We cannot collect all sets in a single set, so to formalise the category of sets we need at least two notions of ``collection'' (we use HOL sets and ZFC sets).
\item
We need to reconcile ZFC sets with the existing HOL types (which represent sets at the meta-level) by choosing representative ZFC sets for small HOL sets.
\end{itemize}
