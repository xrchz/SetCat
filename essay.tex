...
%}
\section{Composable Morphisms and Categories}%1214/1400%{
...
However, this notion is problematic for several reasons, one of which is the fact that objects in our categories must all have the same type, but we make use of functors between categories of different types.
...
Extensionality for functors enables us to characterise equality of functors as follows
\begin{align*}
\vdash\forall{f,g}.\;&\mathtt{is\_functor}\;f\land\mathtt{is\_functor}\;g\land(f.\mathtt{dom} = g.\mathtt{dom})\land(f.\mathtt{cod} = g.\mathtt{cod})\land{}\\
&(\forall{h}.\;h\in f.\mathtt{dom}.\mathtt{mor}\implies(f.\mathtt{map}\;h= g.\mathtt{map}\;h))\implies (f = g)
\end{align*}
For any category we can define an identity functor that acts on morphisms (and on objects) like the identity function.
Our theorem about functor equality is used to prove, for example, 
\[\vdash\forall{f,c}.\;\mathtt{is\_functor}\;f\land(c = f.\mathtt{dom})\implies(f \circ\mathtt{id\_functor}\;c = f)\]
because our definition of functor composition (the $\circ$ operator above) produces an extensional functor, so comparison with $f$ depends only on categorical properties.
We have defined, for any two categories, the category of functors between them with natural transformations as morphisms.
...
%}
\section{A Category of Classes and of Sets}%483/1400%{
\label{Set}
...
If $c.\mathtt{mor}$ is itself a set, then there is no category whose morphisms are all the functions between sets, since no set can contain all functions <ref>, so $\Set$ cannot be formalised.
If we define categories in terms of homs instead, a similar problem comes up when considering, for example, the category of presheaves over the category of sets <really?>.
...
This method of accessing set theory directly within the HOL logic was introduced by Mike Gordon \cite{DBLP:conf/tphol/Gordon96} (and independently by Bob Solovay, see~\cite{DBLP:conf/ictac/Obua06}) and is implemented in both HOL4 and Isabelle~\cite{DBLP:conf/ictac/Obua06}.
