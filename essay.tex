...
%}
\section{Binary Products and Other Limits}%359/250%{
\label{limits}
...
The fact that $\mathtt{set\_cat}$ has binary products is reassuring for $\mathsf{zfset}$ approach to sets.
We would not expect to be able to prove that $\mathtt{ens\_cat}\;\mathtt{UNIV}$ has products.

An important universal construction that is not an instance of a limit is the exponential object.
...
We can easily define the binary product functor using products as above.
However, the best way to treat adjunctions is not obvious.
We could define exponential objects directly, ignoring the adjunction, but to do so is to miss out on opportunities for abstraction, for proving a theorem once and using it in many different contexts.
%}
\section{Conclusions}%363/300%{
...
\item
We cannot collect all sets up as a single set, so to formalise the category of sets we need at least two notions of ``collection'' (we use HOL sets and ZFC sets).
\item
